...
let out = document.getElementById('logout') as HTMLButtonElement;
...
let getOne = JSON.parse(sessionStorage.getItem('login') || '[]')[0];
console.log(getOne);
let logoOne = getOne.email.split('@')[0];
let showOne = logoOne.charAt(0).toUpperCase() + logoOne.slice(1);
hello.textContent = `Hello, ${showOne}`;
